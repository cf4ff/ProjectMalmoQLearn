...
class TabQAgent(object):
    ...
    def updateQTable(self, reward, current_state, prev_state, prev_a):
        v_value = max(self.q_table[current_state][0], self.q_table[current_state][1], self.q_table[current_state][2],
                      self.q_table[current_state][3])
        self.q_table[prev_state][prev_a] = (1-self.alpha) * self.q_table[prev_state][prev_a] + self.alpha*(reward + self.gamma * v_value)
        return
    ...
    def act(self, world_state, agent_host, current_r):
        ...
        explore = random.randrange(0, 100, 1)/100
        next_action = -1
        ...
        if explore <= self.epsilon:
            ...
            best_options = []
            max_score = max(self.q_table[current_s])
            for k in range(4):
                if self.q_table[current_s][k] == max_score:
                    best_options.append(k)
            next_action = random.choice(best_options)
